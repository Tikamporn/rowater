...
<div class="w3-container">
	<div class="w3-row">
		...
		<div class="w3-col m4 s4 w3-round-xlarge">
			...
			<br>
			<form class="w3-container" role="form" method="POST" action="{{ url('/team/add/member_addAction') }}/{{$id}}" accept-charset="utf-8" onsubmit="return confirm('Do you really want to submit the form?');">
				    ...
				    <div class="w3-modal-content">
				      	<div class="w3-container">
					        ...
					        <ul class="w3-ul">
						        ...
						        @foreach($user as $users)		      	
				        			<li>
				        				<input class="w3-check" type="checkbox" name="member[]" <?php if (in_array($users['id'], $member)) echo 'checked="checked"'; ?> value="{{$users->id}}">
										<label class="w3-text-blue w3-large">{{$users->name}}</label>
									</li>
						        @endforeach
					        </ul>
